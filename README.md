...
按基础款分组的留出集包含 72 个基础款、216 张图片。LoRA 的目标结构解析率和
六字段完整率均为 100%，六字段常规宏平均为 78.86%，平衡宏平均为 45.11%，
全字段同时通过率为 16.67%。不读取图片的多数类常量基线常规宏平均为 78.47%，
说明当前权重已经打通西装输出协议，但图像属性区分能力仍需继续提升。

216 个预测均完成 K62/Warp 缝合，缝合失败率为 0.00%。该结果与 ChatGarment
论文使用同名指标，但测试集及固定装配拓扑不同；CD/F-Score 需要逐样本三维西装
真值，当前不报告。详见 [评测方法](suit_finetune/EVALUATION.md)、
[完整结果](suit_finetune/RESULTS.md)和
[机器可读摘要](suit_finetune/results/suit_lora_v1_summary.json)。
