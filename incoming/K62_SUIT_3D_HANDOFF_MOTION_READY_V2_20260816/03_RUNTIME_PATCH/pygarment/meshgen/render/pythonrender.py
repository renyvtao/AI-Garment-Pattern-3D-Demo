...
def create_camera(pyrender, pyrender_body_mesh, scene, side, camera_location=None):
    ...
    if side == 'back':
        ...
        camera_pose = rotate_matrix_y(camera_pose, -45)
    elif side == 'right45':
        camera_pose = rotate_matrix_y(camera_pose, 45)
...
def load_meshes(paths:PathCofig, body_v, body_f):
    ...
    material_arg = None
    if hasattr(garm_mesh.visual, 'material'):
        ...
        # Simulation frame OBJ files may intentionally omit UV/MTL data.
        # Preserve a visible garment with the native renderer in that case.
        material_arg = pyrender.MetallicRoughnessMaterial(
            baseColorFactor=(0.22, 0.07, 0.14, 1.0), metallicFactor=0.0, roughnessFactor=0.85)
        material_arg.doubleSided = True

    pyrender_garm_mesh = pyrender.Mesh.from_trimesh(garm_mesh, material=material_arg, smooth=True)
