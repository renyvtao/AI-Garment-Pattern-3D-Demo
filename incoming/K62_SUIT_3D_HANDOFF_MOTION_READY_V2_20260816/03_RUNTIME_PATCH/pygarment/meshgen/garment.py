...
from pygarment.meshgen.topology_self_collision import LocalTopologySelfCollisionFilter
from pygarment.pattern.core import BasicPattern

# Optional adapter hook.  Default is disabled and preserves all existing
# simulation behavior.  K62-specific runners may set this before Cloth init.
DEFAULT_EXTERNAL_FORCE_CALLBACK = None
...
class Cloth:
    def __init__(self, 
        ...
        self.frame = -1
        self.external_force_callback = DEFAULT_EXTERNAL_FORCE_CALLBACK
        ...
        # Generic, optional local-topology point-triangle self-contact filter.
        self.local_topology_self_collision_filter = None
        if config.enable_local_topology_self_collision_filter:
            spec_path = self.paths.g_specs
            if not spec_path.exists() and hasattr(self.paths, 'in_g_spec'):
                spec_path = self.paths.in_g_spec
            self.local_topology_self_collision_filter = LocalTopologySelfCollisionFilter(
                spec_path=spec_path,
                mesh_resolution=config.resolution_scale,
                model=self.model,
                device=self.device,
                rings=config.local_topology_self_collision_rings,
                panel_patterns=config.local_topology_self_collision_panel_patterns,
            )
            print(
                f'{self.name}::LocalTopologySelfCollisionFilter::'
                f'panels={self.local_topology_self_collision_filter.selected_panels}; '
                f'rings={self.local_topology_self_collision_filter.rings}; '
                f'forbidden_pairs={self.local_topology_self_collision_filter.forbidden_pair_count}'
            )
    ...
    def _sim_frame_with_substeps(self):
        ...
        if self.local_topology_self_collision_filter is not None:
            self.local_topology_self_collision_filter.apply(
