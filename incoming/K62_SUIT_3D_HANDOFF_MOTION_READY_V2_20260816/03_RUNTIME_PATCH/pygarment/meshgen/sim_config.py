...
class SimConfig:
    def __init__(self, sim_props):
        ...
        # Basic setup
        self.sim_fps = 60.0
        # Configurable numerical resolution; default preserves legacy behavior.
        self.sim_substeps = int(self.get_sim_props_value(
            sim_props, 'sim_substeps', 10))
        ...
        # Optional local topology exclusion for point-triangle self-contact.
        # Generic simulator feature: garment-specific panel selection lives in YAML.
        self.enable_local_topology_self_collision_filter = self.get_sim_props_value(
            sim_props_option,
            'enable_local_topology_self_collision_filter', False)
        self.local_topology_self_collision_rings = int(self.get_sim_props_value(
            sim_props_option,
            'local_topology_self_collision_rings', 2))
        self.local_topology_self_collision_panel_patterns = self.get_sim_props_value(
            sim_props_option,
